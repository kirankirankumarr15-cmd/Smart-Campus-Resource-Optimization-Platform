      ...
      <div className="w-full max-w-[400px] mx-4 bg-white rounded-card p-8 shadow-2xl">
        ...
        {showGooglePicker && (
          <div className="fixed inset-0 bg-navy/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="bg-white w-full max-w-[360px] rounded-card shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
              <div className="p-6 pb-2 text-center">
                <div className="flex justify-center mb-3">
                  <svg width="24" height="24" viewBox="0 0 24 24"><path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/><path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/><path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/><path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/></svg>
                </div>
                <h2 className="text-xl font-medium text-navy">Choose an account</h2>
                <p className="text-sm text-textmute mt-1">to continue to Campus Autopilot</p>
              </div>
              
              <div className="mt-4 px-2 mb-6">
                {[
                  { email: '[email protected]', name: 'Campus Management', role: 'campus_management', initial: 'C', color: 'bg-orange' },
                  { email: '[email protected]', name: 'IT Admin', role: 'it_administration', initial: 'I', color: 'bg-blue-600' },
                  { email: '[email protected]', name: 'Academic Team', role: 'academic_scheduling', initial: 'A', color: 'bg-green-600' },
                  { email: '[email protected]', name: 'Kiran Kumar', role: 'student', initial: 'K', color: 'bg-purple-600' }
                ].map((acc) => (
                  <button
                    key={acc.email}
                    onClick={() => {
                  ...
                  >
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center text-white text-xs font-bold ${acc.color}`}>
                      {acc.initial}
                    </div>
                    <div className="text-left">
                      <p className="text-sm font-medium text-navy leading-tight">{acc.name}</p>
                      <p className="text-xs text-textmute">{acc.email}</p>
                    </div>
                  </button>
                ))}
                ...
                >
                  ...
                  <p className="text-sm font-medium text-navy">Use another account</p>
                </button>
              </div>
              
              <div className="bg-gray-50 px-6 py-4 text-[11px] text-textmute leading-relaxed">
                To continue, Google will share your name, email address, language preference, and profile picture with Campus Autopilot.
              </div>
            </div>
          </div>
        )}
